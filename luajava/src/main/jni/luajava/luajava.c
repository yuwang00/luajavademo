  ...
  ( JNIEnv * env , jobject jobj , jobject cptr , jint stateId )
{
  ...
  {
    tempClass = ( *env )->FindClass( env , ""PACKAGE_NAME"LuaJavaAPI" );
  ...
  {
    tempClass = ( *env )->FindClass( env , ""PACKAGE_NAME"JavaFunction" );
  ...
  (JNIEnv * env , jobject jobj , jobject cptr , jobject obj )
{
   ...
   *userData = globalRef;
   ...
   if ( lua_setmetatable( L , -2 ) == 0 )
   {
      ( *env )->ThrowNew( env , ( *env )->FindClass( env , ""PACKAGE_NAME"LuaException" ) ,
                          "Index is not a java object" );
   }
}
  ...
  (JNIEnv * env , jobject jobj)
{
   ...
   tempClass = ( *env )->FindClass( env , ""PACKAGE_NAME"CPtr" );
